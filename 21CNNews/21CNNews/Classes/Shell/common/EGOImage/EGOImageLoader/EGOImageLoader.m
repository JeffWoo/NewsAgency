...
- (void)loadImageForURL:(NSURL*)aURL style:(NSString*)style styler:(UIImage* (^)(UIImage* image))styler completion:(void (^)(UIImage* image, NSURL* imageURL, NSError* error))completion {
	...
	if(anImage) {
		...
		dispatch_async(kStylerQueue, ^{
			...
			[[EGOCache currentCache] setImage:image forKey:keyForURL(aURL, style) withTimeoutInterval:6048000000];
...
- (void)imageLoadConnectionDidFinishLoading:(EGOImageLoadConnection *)connection {
	...
	if(!anImage) {
		...
		[[EGOCache currentCache] setData:connection.responseData forKey:keyForURL(connection.imageURL,nil) withTimeoutInterval:6048000000];
	...
	for(NSString* styleKey in connection.handlers) {
		...
		if(!error && image && styler) {
			dispatch_async(kStylerQueue, ^{
				...
				[[EGOCache currentCache] setImage:anImage forKey:keyForURL(imageURL, styleKey) withTimeoutInterval:6048000000];
